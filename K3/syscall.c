...
sys_read(read_args_t *arg)
{
        ...
        //return -1;
        read_args_t readArgs;
        int retVal = copy_from_user(&readArgs,arg,sizeof(readArgs));
        ...
        void *buf = page_alloc();
        int bytes = do_read(readArgs.fd, buf, readArgs.nbytes);
        ...
        retVal = copy_to_user(arg->buf, buf, bytes);
...
sys_write(write_args_t *arg)
{
        ...
        //return -1;
        write_args_t writeArgs;
        int retVal = copy_from_user(&writeArgs, arg, sizeof(writeArgs));
        ...
        void *buf = page_alloc();
        retVal = copy_from_user(buf, writeArgs.buf, writeArgs.nbytes);
        ...
        int bytes = do_write(writeArgs.fd, buf, writeArgs.nbytes);
        if(bytes<0){
            curthr->kt_errno = -bytes;
            page_free(buf);
            dbg(DBG_PRINT, "(GRADING3A)\n");
...
sys_getdents(getdents_args_t *arg)
{
        ...
        //return -1;
        getdents_args_t getdentsArgs;
        int retVal = copy_from_user(&getdentsArgs, arg, sizeof(getdentsArgs));
        ...
        dirent_t dir;
        int num = (int) getdentsArgs.count/sizeof(dirent_t);
        int bytes = 0;
        int totalBytes = 0;

        for(int i = 0; i < num; i++){
            bytes = do_getdent(getdentsArgs.fd, &dir);

            if(bytes > 0){
                retVal = copy_to_user(getdentsArgs.dirp+i, &dir, bytes);
